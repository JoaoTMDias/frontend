 ...
 *
 * ```js
 * import { getValue } from '@jtmdias/js-utilities';
 *
 * const OBJ = {
 *   a: {
 *    b: {
...
export function getValue<T, R = unknown>(
	...
	if (!isUndefined(DEFAULT_VAL)) {
		console.warn(
			`[@jtmdias/js-utilities] The path ${path} does not exist on the object. Using ${DEFAULT_VAL} instead.`
		);
		return DEFAULT_VAL as R;
	}

	console.error(`[@jtmdias/js-utilities] The path ${path} does not exist on the object.`);
