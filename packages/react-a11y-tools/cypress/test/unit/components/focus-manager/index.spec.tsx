...
describe("FocusManager", () => {
	describe("focus containment", () => {
			...
			cy.mount(
				<FocusManager>
					<fieldset>
						...
						<input id="input-1" data-testid="js-input-1" />
						<label htmlFor="input-2"></label>
						<input id="input-2" data-testid="js-input-2" />
						<label htmlFor="input-3"></label>
						<input id="input-3" data-testid="js-input-3" />
					</fieldset>
				</FocusManager>
			);

			// Should contain the focus travelling forward
			cy.findByTestId("js-input-1").should("have.focus").tab();
			cy.tabUntil(() => cy.findByTestId("js-input-1"));

			// As well at travelling backwards
			cy.findByTestId("js-input-1").tab({ shift: true });
			cy.tabUntil(() => cy.findByTestId("js-input-1"), true);
		});
		...
		it("should do nothing if a modifier key is pressed", () => {
			...
			cy.findByTestId("js-input-1").focus();
			cy.findByTestId("js-input-1").should("have.focus").realPress(["Alt", "Meta", "P"]);
		...
		it("should work with multiple focus scopes", () => {
			...
			cy.tabUntil(() => cy.get("@closeDialog"));
			cy.get("@closeDialog").focus().tab({ shift: true });
			...
			cy.findByTestId("js-input-6").should("have.focus");
		...
		it("uses document.activeElement instead of e.relatedTarget on blur to determine if focus is still in scope", () => {
			...
			cy.findByTestId("js-input-2").focus();
	...
	describe("focus restoration", () => {
		...
		it("should move focus to the next element after the previously focused node on Tab", () => {
			...
			cy.mount(<Test />).then(({ rerender }) => {
				...
				cy.findByTestId("js-input-1").should("have.focus");
				cy.findByTestId("js-input-3").focus();
				cy.findByTestId("js-input-3").tab();
		...
		it("should move focus to the previous element after the previously focused node on Shift+Tab", () => {
			...
			cy.mount(<Test />).then(({ rerender }) => {
				...
				cy.findByTestId("js-input-1").should("have.focus").tab({ shift: true });
				cy.findByTestId("after").should("have.focus");
			});
		});
			...
			cy.mount(
				<FocusManager autoFocus>
					...
					<fieldset>
						...
						<input id="input-1" data-testid="js-input-1" />
						<label htmlFor="input-2"></label>
						<input id="input-2" data-testid="js-input-2" autoFocus />
						<label htmlFor="input-3"></label>
						<input id="input-3" data-testid="js-input-3" />
					</fieldset>
				</FocusManager>
