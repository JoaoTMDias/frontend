...
function findIndexOfVeryFirstActiveTabStop(tabStops: State["tabStops"]): number {
	...
	switch (action.type) {
		case ActionType.REGISTER_TAB_STOP: {
			...
			for (let i = state.tabStops.length - 1; i >= 0; --i) {
				...
				if (loopTabStop.id === newTabStop.id) {
					console.warn(`'${newTabStop.id}' tab stop already registered`);
					return state;
				}
		...
		case ActionType.UNREGISTER_TAB_STOP: {
			...
			if (newTabStops.length === state.tabStops.length) {
				console.warn(`'${id}' tab stop already unregistered`);
				return state;
			}
			return {
				...state,
		...
		case ActionType.TAB_STOP_UPDATED: {
			...
			if (index === -1) {
				console.warn(`'${id}' tab stop not registered`);
				return state;
			}
		...
		case ActionType.KEY_DOWN: {
			...
			switch (navigation) {
				...
				case Navigation.PREVIOUS_ROW: {
					if (isNil(currentTabStop.rowIndex) || currentTabStop.rowIndex === 0) {
						return state;
					}
				...
				case Navigation.LAST_IN_ROW:
					{
						if (isNil(currentTabStop.rowIndex)) {
							return state;
						}
						const rowStartMap = state.rowStartMap || createRowStartMap(state);
