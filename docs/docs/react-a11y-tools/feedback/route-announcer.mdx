...
import { RouteAnnouncer } from "@jtmdias/react-a11y-tools";
import { createHistory, LocationProvider } from "react-router-dom";
...
The process of using the `RouteAnnouncer` with React Router is very similar to the one with `react-router-dom`.
...
This one was even simpler than `react-router-dom`!
